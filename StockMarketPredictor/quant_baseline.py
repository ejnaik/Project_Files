...
def _run_forecast(ticker: str) -> tuple[dict, pd.DataFrame]:
    """Shared implementation behind `predict_stock_baseline` and
    `predict_stock_baseline_with_history`. Trains the XGBRegressor and
    returns both the forecast result dict AND the full feature dataframe
    (so callers that need the historical Close series for charting - see
    `chart_utils.py` - don't have to re-download/re-engineer it themselves
    and risk it drifting from the exact data the model actually trained on).
...
def predict_stock_baseline(ticker: str) -> dict:
    ...
    Note: this is the function bound to Phase 4's `get_quantitative_forecast`
    tool - its return shape must stay exactly this dict, since the LLM tool
    schema and downstream Pydantic models depend on it. Callers that also
    need the historical price series (e.g. for a chart) should use
    `predict_stock_baseline_with_history` instead of re-deriving it.
    """
    result, _df = _run_forecast(ticker)
    return result


def predict_stock_baseline_with_history(ticker: str) -> tuple[dict, "pd.Series"]:
    """Same forecast as `predict_stock_baseline`, but also returns the
    historical `Close` price series (pandas Series, indexed by date) that
    was actually used to train the model - including today's live
    inference row - for callers that want to visualize it (see
    `chart_utils.generate_forecast_chart_base64`).

    Returns
    -------
    tuple[dict, pd.Series]
        (forecast result dict - identical shape to predict_stock_baseline,
         Close price series covering the full lookback window)
    """
    result, df = _run_forecast(ticker)
    return result, df["Close"]

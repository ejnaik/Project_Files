...
in-memory Chroma vector store using OpenAI embeddings, and exposes
...
# "text-embedding-3-small" is OpenAI's cheap, current-generation embedding
# model - plenty for this project's scale. Override via env var if you want
# the higher-dimensional "text-embedding-3-large" instead.
...
def _get_embeddings() -> OpenAIEmbeddings:
    ...
    # we check explicitly here so a missing key fails fast with a clear
    # message instead of a confusing SDK error deep inside the embed call.
    if not os.getenv("OPENAI_API_KEY"):
        raise EnvironmentError(
            "OPENAI_API_KEY is not set. Export it before building the "
            "vector store (OpenAIEmbeddings requires it)."
        )
...
def build_vector_store(
    ...
    embeddings : OpenAIEmbeddings, optional
        ...
        a stub). Defaults to `text-embedding-3-small`.
    persist : bool

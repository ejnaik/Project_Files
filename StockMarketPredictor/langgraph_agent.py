...
# "gpt-4o-mini" is a cheap, fast OpenAI chat model that supports tool calling
# via .bind_tools(). Override via env var if you want a larger model (e.g.
# for higher-stakes analysis) without editing code.
...
def _build_llm_with_tools() -> ChatOpenAI:
    # langchain_openai reads OPENAI_API_KEY from the environment itself, but
    # we check explicitly here so a missing key fails fast at warm-up with a
    # clear message instead of a confusing SDK error on the first
...
def warm_up() -> None:
    ...
    client-construction cost, and so a missing OPENAI_API_KEY surfaces as
...
def run_financial_agent(user_query: str) -> str:
    ...
    plain string. `ChatOpenAI` (the active provider here) typically returns
    one, but other providers - e.g. Gemini via `langchain_google_genai`,
    which this project ran on earlier - can return a list of content blocks
    instead, e.g. `[{"type": "text", "text": "..."}]`, even for a
    single-part response. That mismatch broke this exact function once,
    when the LLM was swapped to Gemini: FastAPI's `PredictResponse` model
    ...
    not `.content`) normalizes both shapes into a plain string, so we keep
    using it here even on OpenAI - it's a no-op when content is already a
    string, and it means swapping providers again won't silently reintroduce
    that bug. This fix belongs here rather than in main.py, since main.py
    should not need to know about a specific provider's message internals.
    """
...
if __name__ == "__main__":
    # Requires OPENAI_API_KEY, a live network path to yfinance, and a
